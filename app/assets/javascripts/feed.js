...
function formatDiv(rep) {
  ...
  var div = "<tr><td style='text-align:center'><img style='width:25px;display:inline-block;padding: 0 15px; 'src='" + img + "'><div data='" + strDate + "'style='padding-top:3px;text-align:left'><b>Description:</b> " + desc + "<br>" +
    "<b>Time:</b> " + strTime + "</div><hr/></td></tr>";
  timeLength = strDate.length;
  return div;
...
function appendHelper(arr) {
  ...
  if (!arr.length <= constants.numbDivsToLoad - oldLen || loadedSmells.length < load) {
    button = "<tr id='buttonRow'><td colspan=\"2\" style='font-size:7px;text-align:center'><button onclick='loadPrevious()'id='load-button'>Load Previous Day</button><hr/></td></tr>";
  }
  //onclick empty
  ...
  if (arr[0] != undefined) {
    var strDate = arr[0].substring(arr[0].indexOf("data='") + "data='".length, arr[0].indexOf("data='") + timeLength + "data='".length);
  }
  if (isSetUp) {
    ...
    tab.id = "tab";
    if (arr[0] != undefined) {
      arr.unshift("<tr id='dayLabFirst'><td colspan=\"2\" style='font-size:13px;text-align:center'><div><b>" + strDate + "</b></div><hr/></td></tr>")
    }
  }
  //always prepend the smell reports
  if (loadPrev) {
    arr.unshift("<tr class='dayLab'><td colspan=\"2\" style='font-size:13px;text-align:center'><div><b>" + strDate + "</b></div><hr/></td></tr>");
    loadPrev = false;
  }
  ...
  if (isLoadMore) {
    ...
    if (newReps) {
      saveDate = $("#dayLabFirst")[0].outerHTML;
      $("#dayLabFirst").replaceWith("");
      $("#tab").prepend(saveDate);
    }
  ...
  if (isSetUp) {
    ...
    if (arr.length <= constants.numbDivsToLoad - oldLen) {
      ...
      $("#load-button").replaceWith("<button onclick='loadPrevious()'id='load-button'>Load Previous Day</button>");
    }
  }
    ...
    if (areNewReps) {
      ...
      $.getJSON(req, function(data2) {
        ...
        for (var k = 0; k < data2.length; k++) {
          var isUn = true;
          for (var i = 0; i < loadedSmells.length; i++) {
            if ((loadedSmells[i].observed_at == data2[k].observed_at && loadedSmells[i].smell_description == data2[k].smell_description)) {
              isUn = false;
            }
          }
          if (isUn) {
            arrNewReps.push(data2[k]);
          }
        }
        arrNewReps.reverse();
        loadedSmells = arrNewReps;
...
function loadPrevious() {
  ...
  var localMidnight = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 0, 0, 0); //the most recent midnight
  localMidnight = localMidnight.getTime() / 1000; //convert to seconds
  var url = apiURL + "" + constants.regionId + "&start_time=" + (localMidnight - (24 * 60 * 60 * (daysLoaded + 1))) + "&end_time=" + (localMidnight - (24 * 60 * 60 * daysLoaded)); //url to get reports in last offset seconds
  if ((localMidnight - (24 * 60 * 60 * (daysLoaded + 1))) > 1532491100) { //1472688000
    $("#load-button").replaceWith("<button onclick=''id='load-button'>Loading...</button>");
    $.getJSON(url, function(data) {
      if (data.length > 0) {
        data.sort(function(a, b) {
          return a.observed_at - b.observed_at;
    ...
    $("#buttonRow").replaceWith("<tr id='buttonRow'><td colspan=\"2\"><h3 style='text-align:center'>There are no more smell reports</h3><hr/></td></tr>");
