class SmellReportTracker < ActiveRecord::Base
  ...
  # listening_for_smell_reports: (boolean) indicates the state of listening for smell reports for immediate callback (push notification)
  # generating_hourly_summary_for_smell_reports: (boolean) indicates that we are waiting to calculate an hourly summary (since the last smell report that was handled in "listening_for_smell_reports")
  # last_smell_report_tracker_summary: (integer) the timestamp of the last smell report
  ...
  def self.is_listening_for_smell_reports?
    if Rails.cache.read("listening_for_smell_reports").nil?
      ...
      hour = ApplicationHelper.get_time_by_alias("PGH")
  ...
  def self.is_generating_hourly_summary?
    if Rails.cache.read("generating_hourly_summary_for_smell_reports").nil?
      Rails.cache.write("generating_hourly_summary_for_smell_reports", false)
    end
    return Rails.cache.read("generating_hourly_summary_for_smell_reports")
  end


  def self.generating_hourly_summary(flag)
    Rails.cache.write("generating_hourly_summary_for_smell_reports", flag)
  end
